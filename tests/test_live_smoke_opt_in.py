...
class FakeItem:
    ...
    def iter_markers(self, name: str | None = None) -> list[object]:
        if self.marked and name in (None, conftest.LIVE_CODEX_CLI_MARKER):
            return [object()]
        return []
...
def test_collection_skips_marked_tests_when_live_codex_cli_is_not_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    conftest.pytest_collection_modifyitems(config=None, items=[marked, unmarked])  # type: ignore[arg-type]
